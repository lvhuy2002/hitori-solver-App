...
public class OriginCESolver extends BaseSolver {
    public String status = "UNSAT";
    public ArrayList<int[]> cnfClause;
    OriginCESolver(int matrixSize, int[][] input) {
    ...
    public void generateThirdRuleClause() {
        ...
        //Tính chất bắc cầu
        for (int u = 1; u <= matrixSize; u++) {
            for (int v = 1; v <= matrixSize; v++) {
        ...
        //Các ô biên ko được connect với nhau
        for (int u = 1; u <= matrixSize; u++) {
            for (int v = 1; v <= matrixSize; v++) {
                for (int i = 1; i <= matrixSize; i++) {
                    for (int j = 1; j <= matrixSize; j++) {
                        if ((i == 1 || j == 1 || i == matrixSize || j == matrixSize) 
                            && (u == 1 || v == 1 || u == matrixSize || v == matrixSize)) {
                                int[] clause = {-makeIdForC(u, v, i, j)};
        ...
        for (int i = 1; i <= matrixSize; i++) {
            for (int j = 1; j <= matrixSize; j++) {
                ...
                if (i == 1 && (j != 1 && j != matrixSize) ) {
                    makeCNFToMakeOneDirection(i, j, i + 1, j - 1, i + 1, j + 1);
                }
                if (i == matrixSize && (j != 1 && j != matrixSize)) {
                    makeCNFToMakeOneDirection(i, j, i + 1, j - 1, i + 1, j + 1); 
                }
                if (j == 1 && (i != 1 && i != matrixSize) ) {
                    makeCNFToMakeOneDirection(i, j, i - 1, j + 1, i + 1, j + 1);
                }
                if (j == matrixSize && (i != 1 && i != matrixSize) ) {
                    makeCNFToMakeOneDirection(i, j, i - 1, j - 1, i + 1, j - 1);
                }
            }
        }
